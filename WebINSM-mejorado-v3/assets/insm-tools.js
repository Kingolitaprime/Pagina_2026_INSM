/* ==========================================================================
   ...
   - Botón flotante: Modo blanco y negro (accesibilidad, persistente)
...
(function () {
	...
	style.textContent =
		...
		'.insm-fab.insm-fab-active{background:#2b2b2b;}' +
		'.insm-fab[data-tip]{position:relative;}' +
		'html.insm-grayscale .insm-fab-group{filter:none !important;}' + /* mantiene los botones legibles en modo B/N */
		...
		'html.insm-grayscale{filter:grayscale(1) contrast(1.04) !important;}';
	...
	grayscaleBtn.className = 'insm-fab';
	grayscaleBtn.setAttribute('aria-pressed', 'false');
	// Icono de contraste (medio círculo relleno) dibujado a mano
	grayscaleBtn.innerHTML =
		'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9" fill="none" stroke="currentColor" stroke-width="1.8"/>' +
	...
	fabGroup.appendChild(grayscaleBtn);
	fabGroup.appendChild(searchBtn);

	/* -------------------- 5) Modo blanco y negro -------------------- */
	function setGrayscale(on) {
		document.documentElement.classList.toggle('insm-grayscale', on);
		grayscaleBtn.setAttribute('aria-pressed', on ? 'true' : 'false');
		grayscaleBtn.classList.toggle('insm-fab-active', on);
		grayscaleBtn.title = on ? 'Desactivar modo blanco y negro' : 'Activar modo blanco y negro';
		grayscaleBtn.setAttribute('aria-label', grayscaleBtn.title);
		try {
			localStorage.setItem('insm-grayscale', on ? 'on' : 'off');
		} catch (e) {
			/* localStorage no disponible: el modo no persistirá entre páginas */
		}
	}

	grayscaleBtn.addEventListener('click', function () {
		setGrayscale(!document.documentElement.classList.contains('insm-grayscale'));
	});

	// Sincroniza el estado visual del botón con la clase ya aplicada de forma temprana (ver snippet en <head>)
	setGrayscale(document.documentElement.classList.contains('insm-grayscale'));

	/* -------------------- 6) Buscador / directorio del sitio -------------------- */
	...
	/* -------------------- 7) Envío genérico de formularios de preinscripción -------------------- */
